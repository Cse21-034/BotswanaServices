...
const BusinessDashboardPage: FC<BusinessDashboardPageProps> = ({ }) => {
  ...
  // Fetch listings and subscription when businessData.id changes
  useEffect(() => {
    if (businessData.id) {
      fetchListings();
      fetchPropertyListings();
      fetch(`/api/subscriptions/status?businessId=${businessData.id}`)
        .then(r => r.json())
        .then(d => { if (d.success) setSubscriptionTier(d.subscription?.tier || 'WILD_HORSES'); })
        .catch(() => setSubscriptionTier('WILD_HORSES'));
    }
  }, [businessData.id]);
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-lg border border-neutral-200 dark:border-neutral-700">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
                WhatsApp Number
                {subscriptionTier !== 'DESERT_LIONS' && <span className="ml-2 text-xs text-amber-600 font-normal">(Desert Lions plan)</span>}
              </label>
              <Input
                name="whatsappNumber"
                ...
                className="w-full"
                disabled={subscriptionTier !== 'DESERT_LIONS'}
              />
              <p className="text-xs text-neutral-400 mt-1">Customers will see a WhatsApp chat button on your profile</p>
            </div>
            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
                Video Introduction
              ...
              {subscriptionTier === 'DESERT_LIONS' ? (
                <VideoUploader
                  currentUrl={businessData.videoUrl || ""}
                  onUploadComplete={(url) => setBusinessData(prev => ({ ...prev, videoUrl: url }))}
                  onDelete={() => setBusinessData(prev => ({ ...prev, videoUrl: "" }))}
                />
              ) : (
                <div className="w-full border-2 border-dashed border-neutral-200 dark:border-neutral-700 rounded-xl p-8 text-center bg-neutral-50 dark:bg-neutral-800/50 opacity-60 cursor-not-allowed">
                  <p className="text-sm text-neutral-400">Upgrade to Desert Lions to upload a video introduction</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="mt-8 pt-8 border-t border-neutral-200 dark:border-neutral-700">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                { name: 'facebook', label: 'Facebook', placeholder: 'https://facebook.com/yourbusiness', icon: '📘' },
                { name: 'instagram', label: 'Instagram', placeholder: 'https://instagram.com/yourbusiness', icon: '📸' },
                { name: 'twitter', label: 'X (Twitter)', placeholder: 'https://x.com/yourbusiness', icon: '🐦' },
                { name: 'linkedin', label: 'LinkedIn', placeholder: 'https://linkedin.com/company/yourbusiness', icon: '💼' },
                { name: 'tiktok', label: 'TikTok', placeholder: 'https://tiktok.com/@yourbusiness', icon: '🎵' },
                { name: 'youtube', label: 'YouTube', placeholder: 'https://youtube.com/@yourbusiness', icon: '▶️' },
                ...
                <div key={name}>
                  ...
                  <Input
                    name={name}
                    value={(businessData as any)[name] || ''}
                    onChange={handleInputChange}
        ...
        {/* Business Hours — requires Desert Elephants */}
        <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Business Hours" description="Set and display your opening hours so customers know when to reach you.">
        <div className="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-lg border border-neutral-200 dark:border-neutral-700">
          <h2 className="text-2xl font-semibold flex items-center gap-3 mb-6">
        ...
        {/* Services Offered — requires Desert Elephants */}
        <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Services & Profile Details" description="Showcase your services, pricing range, and employee count on your business profile.">
        <div className="bg-white dark:bg-neutral-800 rounded-2xl p-6 shadow-lg border border-neutral-200 dark:border-neutral-700">
          <h2 className="text-2xl font-semibold flex items-center gap-3 mb-6">
    ...
    <div className="nc-BusinessDashboardPage bg-neutral-50 dark:bg-neutral-900 min-h-screen">
      ...
      <main className="lg:container mx-auto px-4 py-8">
        ...
        <div className="mb-8">
          <div className="border-b border-neutral-200 dark:border-neutral-700">
            <nav className="-mb-px flex space-x-8 overflow-x-auto">
              {(() => {
                const tierOrder: Record<string, number> = { WILD_HORSES: 0, DESERT_ELEPHANTS: 1, DESERT_LIONS: 2 };
                const currentOrder = tierOrder[subscriptionTier || 'WILD_HORSES'] ?? 0;
                const tabs = [
                  { id: 'overview', name: 'Overview', icon: ChartBarIcon, requiredOrder: 0 },
                  { id: 'profile', name: 'Profile', icon: BuildingStorefrontIcon, requiredOrder: 0 },
                  { id: 'products', name: 'Products & Listings', icon: DocumentTextIcon, requiredOrder: 1 },
                  // { id: 'property-listings', name: 'Property Listings', icon: HomeIcon, requiredOrder: 1 },
                  { id: 'featured-ads', name: 'Featured Ads', icon: StarIcon, requiredOrder: 0 },
                  { id: 'promotions', name: 'Promotions', icon: SparklesIcon, requiredOrder: 1 },
                  { id: 'membership', name: 'Membership', icon: CheckCircleIcon, requiredOrder: 1 },
                  { id: 'subscription', name: 'My Subscription', icon: CreditCardIcon, requiredOrder: 0 },
                  { id: 'upgrade-plans', name: 'Upgrade Plans', icon: ShoppingBagIcon, requiredOrder: 0 },
                  { id: 'bookings', name: 'Bookings', icon: CalendarDaysIcon, requiredOrder: 0 },
                  { id: 'analytics', name: 'Analytics', icon: EyeIcon, requiredOrder: 2 },
                  { id: 'branches', name: 'Branches', icon: BuildingOfficeIcon, requiredOrder: 1 },
                ];
                return tabs.map((tab) => {
                  const locked = currentOrder < tab.requiredOrder;
                  return (
                    <button
                      key={tab.id}
                      ...
                      className={`whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center gap-2 transition-colors ${activeTab === tab.id
                        ...
                        : locked
                        ? 'border-transparent text-neutral-400 dark:text-neutral-600 hover:text-neutral-500'
                        : 'border-transparent text-neutral-500 hover:text-neutral-700 hover:border-neutral-300 dark:text-neutral-400 dark:hover:text-neutral-300'
                      }`}
                    >
                      <tab.icon className="w-5 h-5" />
                      {tab.name}
                      {locked && <span className="text-amber-500">🔒</span>}
                    </button>
                  );
                });
              })()}
        ...
        <div className="mb-12">
          ...
          {activeTab === 'products' && (
            <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Products & Listings" description="Create and showcase your products and services to attract more customers.">
              {renderListingsTab()}
            </PlanGate>
          )}
          {/* {activeTab === 'property-listings' && (
            <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Property Listings" description="List and manage your rental properties and accommodations to reach more customers.">
              {renderPropertyListingsTab()}
            </PlanGate>
          )} */}
          ...
          {activeTab === 'promotions' && (
            <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Promotions" description="Create special offers and discount campaigns to attract and retain customers.">
              <div className="text-center py-12">
          ...
          {activeTab === 'membership' && (
            <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Membership" description="Manage membership cards and display your business membership credentials to build trust.">
              {businessData && (
                <div>
                  <MembershipsList
          ...
          {activeTab === 'analytics' && (
            <PlanGate requiredTier="DESERT_LIONS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Full Analytics" description="Track views, clicks, customer engagement, and business performance with detailed reports.">
              {businessData?.id && <BusinessAnalytics businessId={businessData.id} />}
            </PlanGate>
          )}
          {activeTab === 'bookings' && (
            <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Bookings Management" description="Track and manage all your customer bookings, confirm appointments, and update booking statuses.">
              <BookingsTab businessId={businessData.id || ''} />
            </PlanGate>
          )}
          {activeTab === 'branches' && (
            <PlanGate requiredTier="DESERT_ELEPHANTS" currentTier={subscriptionTier} businessId={businessData.id || ''} feature="Branch Management" description="Add and manage multiple locations for your business.">
              <div className="text-center py-12">
