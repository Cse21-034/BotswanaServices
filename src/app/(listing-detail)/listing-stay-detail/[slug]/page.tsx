...
const ListingStayDetailPage: FC<ListingStayDetailPageProps> = ({ }) => {
  ...
  const [businessTier, setBusinessTier] = useState<string>('WILD_HORSES');
    ...
    const fetchAndGeocodeBusiness = async () => {
      try {
        ...
        if (data.success && data.business) {
          ...
          // Use the server-computed effective tier — handles all subscription
          // states (ACTIVE, INACTIVE with prior payments, cancelled, etc.)
          setBusinessTier(data.effectiveTier || 'WILD_HORSES');
    ...
    return (
      <div className="listingSection__wrap !space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2 gap-3">
          ...
          {businessTier === 'DESERT_LIONS' && business?.whatsappNumber && (
            <ButtonPrimary
              className="w-full justify-center py-3 !bg-green-600 hover:!bg-green-700"
              onClick={() => window.open(`https://wa.me/${business.whatsappNumber!.replace(/\D/g, '')}`, '_blank')}
            >
    ...
    return (
      <div className="listingSectionSidebar__wrap shadow-xl">
        ...
        <div className="space-y-6">
          ...
          {/* Social Media Links */}
          {(business?.facebook || business?.instagram || business?.twitter || business?.linkedin || business?.tiktok || business?.youtube) && (
            <div className="border-t border-neutral-200 dark:border-neutral-700 pt-6">
              <h3 className="font-semibold text-lg mb-4">Follow Us</h3>
              <div className="flex flex-wrap gap-3">
                {business?.facebook && (
                  <a href={business.facebook} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 px-3 py-2 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 hover:bg-blue-100 transition-colors text-sm font-medium">
                    <i className="lab la-facebook-f text-lg"></i> Facebook
                  </a>
                )}
                {business?.instagram && (
                  <a href={business.instagram} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 px-3 py-2 rounded-xl bg-pink-50 dark:bg-pink-900/20 text-pink-700 dark:text-pink-300 hover:bg-pink-100 transition-colors text-sm font-medium">
                    <i className="lab la-instagram text-lg"></i> Instagram
                  </a>
                )}
                {business?.twitter && (
                  <a href={business.twitter} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 px-3 py-2 rounded-xl bg-neutral-100 dark:bg-neutral-700 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-200 transition-colors text-sm font-medium">
                    <i className="lab la-twitter text-lg"></i> X
                  </a>
                )}
                {business?.linkedin && (
                  <a href={business.linkedin} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 px-3 py-2 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-800 dark:text-blue-300 hover:bg-blue-100 transition-colors text-sm font-medium">
                    <i className="lab la-linkedin-in text-lg"></i> LinkedIn
                  </a>
                )}
                {business?.tiktok && (
                  <a href={business.tiktok} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 px-3 py-2 rounded-xl bg-neutral-900 dark:bg-neutral-700 text-white hover:bg-neutral-700 transition-colors text-sm font-medium">
                    <i className="lab la-tiktok text-lg"></i> TikTok
                  </a>
                )}
                {business?.youtube && (
                  <a href={business.youtube} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-2 px-3 py-2 rounded-xl bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 hover:bg-red-100 transition-colors text-sm font-medium">
                    <i className="lab la-youtube text-lg"></i> YouTube
                  </a>
                )}
              </div>
            </div>
    ...
    <div className="nc-ListingStayDetailPage">
      ...
      <main className="relative z-10 mt-11 flex flex-col lg:flex-row gap-4 xl:gap-6">
        ...
        <div className="w-full lg:w-3/5 space-y-8 lg:space-y-10">
          {renderSection1()}
          {businessTier !== 'WILD_HORSES' && memberships && memberships.length > 0 && (
            <MembershipsDisplay memberships={memberships} />
          )}
          {renderSection2()}
          {businessTier === 'DESERT_LIONS' && business?.videoUrl && (
            <div className="listingSection__wrap">
              <h2 className="text-2xl font-semibold">Video Introduction</h2>
              <div className="w-full aspect-video rounded-xl overflow-hidden bg-black">
                {business.videoUrl.includes('youtube.com') || business.videoUrl.includes('youtu.be') || business.videoUrl.includes('vimeo.com') ? (
                  <iframe
                    src={(() => {
                      const url = business.videoUrl!;
          ...
          {businessTier !== 'WILD_HORSES' && renderSection3()}
          {businessTier !== 'WILD_HORSES' && renderSection4()}
          {renderSection5()}
          {renderSection6()}
          {businessTier !== 'WILD_HORSES' && renderSection7()}
          {renderSection8()}
        </div>
