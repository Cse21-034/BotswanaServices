...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verify checksum using received field order (matches PayGate's calculation)
    ...
    if (!isSuccess) {
      ...
      await prisma.adPayment.update({
        ...
        data: {
          status: 'FAILED',
          failureReason: data.FAILURE_REASON || data.RESULT_DESC || 'Payment declined at gateway',
          paymentGatewayId: data.TRANSACTION_ID || data.REFERENCE,
        },
      });

      // Also mark the subscription as INACTIVE so it doesn't show as live
      await prisma.advertisingSubscription.update({
        where: { id: adPayment.adSubscription.id },
        data: { status: 'INACTIVE' },
      });

      console.log('[AdCallback] ⚠️ Marked ad payment as FAILED and subscription as INACTIVE');
      return new NextResponse('OK');
    }
