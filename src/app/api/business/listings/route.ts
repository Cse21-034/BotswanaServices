...
import { canAddListing } from "@/lib/subscription-access";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check listing limit against subscription plan
    const listingCheck = await canAddListing(businessId);
    if (!listingCheck.allowed) {
      return NextResponse.json({
        error: listingCheck.reason,
        upgradeRequired: true,
        limit: listingCheck.limit,
        current: listingCheck.current,
      }, { status: 403 });
    }
